/**
 * @fileoverview Renders the complete employee onboarding form. It binds every personal, address, employment, reporting, bank, and emergency-contact field to useEmployeeCreate, displays field validation, builds the API payload, saves the record, and navigates to the new employee details page.
 *
 * @description
 ...
 *
 * @module src/components/employee/EmployeeCreate
 */
...
/**
 * Renders the employee create interface and coordinates its presentation behavior.
...
const EmployeeCreate = () => {
	...
	const submitHandler = async (e) => {
		...
		try {
			setSaveError(null);
			setSaving(true);
			const createPayload = buildEmployeeUpdatePayload(employee);
			await employeeApi.createEmployee(createPayload);
			logger.debug("Creating employee", createPayload);
			navigate(`/employees/${employee.employeeId}`);
		} catch (error) {
			logger.error("Unable to create employee", error);
			setSaveError(error);
		} finally {
			setSaving(false);
		}
	...
	if (loadError) {
		return <ErrorState title="Unable to load employee form" message={`Reason : ${loadError.message}`} />;
	}
	...
	return (
		<div className="min-w-0 overflow-y-auto border-t border-slate-200">
			<Popup
				saving={saving}
				error={saveError}
				savingMessage="Creating employee record..."
			...
			<form onSubmit={submitHandler} className="space-y-5 sm:space-y-8">
				...
				<div className="sticky bottom-0 z-10 mt-6 flex flex-col-reverse gap-3 border-t border-slate-200 bg-white p-4 shadow-[0_-4px_12px_rgba(15,23,42,0.06)] sm:mt-8 sm:flex-row sm:justify-end sm:gap-6 sm:p-5">
					<button
						type="submit"
