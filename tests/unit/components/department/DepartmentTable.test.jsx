...
import DepartmentTable from "../../../../src/components/department/DepartmentTable";
...
describe("DepartmentTable Component", () => {
	...
	it("calls onDelete when delete button is clicked", async () => {
		...
		render(<DepartmentTable {...defaultProps} onDelete={onDelete} />);
		const buttons = screen.getByText("Engineering").closest("tr").querySelectorAll("button");
		await user.click(buttons[2]);
		expect(onDelete).toHaveBeenCalledWith(departments[0]);
	});
	...
	it("uses zero metric fallbacks and summarizes additional locations", () => {
		const sparseEmployees = ["Mumbai", "Pune", "Delhi"].map((workLocation, index) => ({
			id: index,
			employment: { departmentId: "D001", status: "Active", workLocation },
		}));
		render(<DepartmentTable {...defaultProps} employees={sparseEmployees} />);
		expect(screen.getByText("+1")).toBeInTheDocument();
