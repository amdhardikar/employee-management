...
import { fetchDashboard } from "../../../src/store/dashboardSlice";
...
vi.mock("../../../src/store/dashboardSlice", () => ({
...
describe("Dashboard", () => {
	...
	it("shows the dashboard error state", () => {
		useSelector.mockReturnValue({ data: null, loading: false, error: new Error("Dashboard unavailable") });
		render(<Dashboard />);
		expect(screen.getByText("Unable to load dashboard")).toBeInTheDocument();
		expect(screen.getByText("Reason : Dashboard unavailable")).toBeInTheDocument();
	});

	it("uses fallback error text and renders an on-leave recent employee", () => {
		useSelector.mockReturnValue({ data: null, loading: false, error: {} });
		const view = render(<Dashboard />);
		expect(screen.getByText("Reason : Unable to load dashboard data.")).toBeInTheDocument();

		useSelector.mockReturnValue({
			data: { ...dashboardMock, recentEmployees: [{ ...dashboardMock.recentEmployees[0], employment: { designation: "Accountant", status: "On Leave" } }] },
			loading: false,
		});
		view.rerender(<Dashboard />);
		expect(screen.getAllByText("On Leave").length).toBeGreaterThan(1);
	});
